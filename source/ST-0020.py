...
for t, noise in enumerate(white_noise):
    ...
    schtochastic_season = 1*time_series[t-30] + 0*white_noise[t-30]
...
# [d=1] random process
#y = pd.Series(time_series).diff(periods=30).dropna()
y = list()
season = 30
for i in range(season, len(time_series)):
    value = time_series[i] - time_series[i - season]
    y.append(value)
axes[3].plot(y, 'o-')
...
# [d=2] random process
#y = pd.Series(time_series).diff(periods=30).dropna()
#y = pd.Series(y).diff(periods=30).dropna()
y = list()
season = 30
for i in range(2*season, len(time_series)):
    value = time_series[i] - 2*time_series[i - 1*season] + time_series[i - 2*season]
    y.append(value)
axes[6].plot(y, 'o-')
